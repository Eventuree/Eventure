  ...
  return createServer({
    ...
    seeds(server) {
      server.db.loadData({
        events: [
          ...
          {
            id: 3, organizer_id: 11, title: "Юнацька ліга футболу", description: "Відкрите тренування та відбір до команди.",
        ...
        profiles: [] // Можна додати тестовий профіль сюди
      });
    },
    ...
    routes() {
      ...
      this.get("/events/:id", (schema, request) => {
        ...
        return schema.db.events.find(eventId) || {
      ...
      this.post("/profiles", (schema, request) => {
        if (request.requestBody instanceof FormData) return { status: 'success' };
      ...
      this.delete("/profiles/:id", (schema, request) => {
        return { status: "success", message: `Profile ${request.params.id} deleted` };
      });
